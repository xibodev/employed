...
tmp="${OUT}.tmp.$$"
{
  ...
  # --- resume artifact storage (Cloudflare R2, S3-compatible) ---
  echo "RESUME_STORAGE_BACKEND=$(ssm_or_default RESUME_STORAGE_BACKEND 'local')"
  echo "RESUME_S3_BUCKET=$(ssm_or_default RESUME_S3_BUCKET '')"
  echo "RESUME_S3_ENDPOINT_URL=$(ssm_or_default RESUME_S3_ENDPOINT_URL '')"
  echo "RESUME_S3_ACCESS_KEY_ID=$(ssm_or_default RESUME_S3_ACCESS_KEY_ID '')"
  echo "RESUME_S3_SECRET_ACCESS_KEY=$(ssm_or_default RESUME_S3_SECRET_ACCESS_KEY '')"
  echo "RESUME_S3_REGION=$(ssm_or_default RESUME_S3_REGION 'auto')"
} > "$tmp"
mv "$tmp" "$OUT"
chmod 600 "$OUT"

...
|----|------|--------|-------|
| BL-OPS-001 | Confirm Google OAuth and reCAPTCHA allowed origins include market hosts if direct auth/submission happens on `mx` or `mz` | `planned` | operator |
| BL-OPS-002 | Replace Stripe test keys with live keys and verify live webhook when monetisation starts | `planned` | operator |
| BL-OPS-003 | Add a user-facing resume download endpoint (R2 storage layer exists; no serve route yet) | `planned` | product/operator |
| BL-OPS-004 | Confirm M-Pesa/e-Mola sandbox or live credentials before real mobile-money processing | `planned` | operator |
